...
namespace DVO
{
                  ...
                  if ( !cvIsNaN ( d0 ) && std::abs ( transformed_d1 - d0 ) <= maxDepthDiff )
                  {
                     ...
                     if ( c != -1 )
                     {
                        ...
                        float exist_d1 = ( float ) ( depth1.at<float> ( exist_v1,exist_u1 ) *
                                                     ( KRK_inv_ptr[6] * exist_u1 + KRK_inv_ptr[7] * exist_v1 + KRK_inv_ptr[8] )
                                                     + Kt_ptr[2] );
                          ...
                          float minDepth, float maxDepth )
   {
      ...
      for ( int y = 0; y < depth0.rows; y++ )
      {
         for ( int x = 0; x < depth0.cols; x++ )
         {
            ...
            if ( !cvIsNaN ( d0 )
                  && ( d0 > maxDepth || d0 < minDepth || d0 <= 0 || ( !validMask0.empty()
                        && !validMask0.at<uchar> ( y,x ) ) ) )

               d0 = std::numeric_limits<float>::quiet_NaN();

            float& d1 = depth1.at<float> ( y,x );
            if ( !cvIsNaN ( d1 )
                  && ( d1 > maxDepth || d1 < minDepth || d1 <= 0 || ( !validMask1.empty()
                        && !validMask1.at<uchar> ( y,x ) ) ) )
                        ...
                        vector<Mat>& pyramidTexturedMask1, vector<Mat>& pyramidCameraMatrix )
   {
      ...
      for ( size_t i = 0; i < pyramidImage1.size(); i++ )
      {
         ...
         const float minScalesGradMagnitude2 = ( float ) ( ( minGradMagnitudes[i] * minGradMagnitudes[i] ) 
   ...
   bool RGBDOdometry ( cv::Mat& Rt, const Mat& initRt,
                           ...
                           int transformType )
   {
      ...
      vector<Mat> pyramidImage0, pyramidDepth0,
             ...
             pyramidCameraMatrix;
      buildPyramids ( image0, image1, 
		      depth0, depth1, cameraMatrix, sobelSize, sobelScale, *minGradientMagnitudesPtr,
